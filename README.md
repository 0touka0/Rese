...
## 注意事項
1. ストレージはS3を使用しているので、ローカル環境では店舗作成時画像の保存が行えません
2. CSVファイルについて
基本ルール
カンマ , はデータ内に使用しないこと
例: 店舗概要に「美味しい、安い」と記述するのではなく、「美味しくて安い」と記述してください。
ヘッダー行は必須です
1行目に列名を記述してください（サンプル参照）。
URLの記述
正しい形式で記述してください（例: http:// または https:// から始まるもの）。
保存形式
文字化けを防ぐため、UTF-8形式で保存してください。
1. 項目は全て入力必須
2. 店舗名：50文字以内
3. 地域：「東京都」「大阪府」「福岡県」のいずれ
4. ジャンル：「寿司」「焼肉」「イタリアン」「居酒屋」「ラーメン」のいずれか
5. 店舗概要：400文字以内
6. 画像URL：jpeg、pngのみアップロード可能